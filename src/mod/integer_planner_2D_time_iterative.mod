...
# arrival check (veloctity and position)
...
subject to arrival_cst_lo{t in 1..n_ts, i in 1..2}: pos[i,t] >= pos_final[i] - (pos_final[i] - pos_cst[2*i-1])*(1-finish[t]);
subject to arrival_cst_hi{t in 1..n_ts, i in 1..2}: pos[i,t] <= pos_final[i] - (pos_final[i] - pos_cst[2*i])*(1-finish[t]);
subject to arrival_logic: sum{t in 1..n_ts} finish[t] = 1;

# obstacle avoidance
subject to obstacle_cst{i in 1..n_ts, j in 1..n_obs, k in 1..n_obs_cst}: (pos[1,i]-obs_centroid[j,1])*sin_obs[k] + (pos[2,i] - obs_centroid[j,2])*cos_obs[k] >= obs_radius[j] - 1000*obs_avoid[i,j,k] - 1000*indicator;
subject to obstacle_avoidance_logic{i in 1..n_ts, j in 1..n_obs}: sum{k in 1..n_obs_cst} obs_avoid[i,j,k] <= n_obs_cst-1+indicator;
