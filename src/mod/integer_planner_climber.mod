...
# No support for the multi-waypoint assignment
...
# wind disturbance
param wind_disturbance{1..2, 1..n_ts}; # wind disturbance at each time step in m/s

param epsilon >0; # weighting on control input

# wall disjunction
param num_disj integer >0; # number of the disjunction
param disjunction{1..num_disj, 1..2}; # disjunction list
...
var db{1..n_ts, 1..num_disj} binary; # disjunction binary decision variable

# minimize time_consumption: sum{t in 1..n_ts} t*finish[t] + epsilon*sum{t in 1..(n_ts-1)} am[t];
...
subject to dynamics{i in 1..2, j in 1..(n_ts-1)}: pos[i,j+1] = pos[i,j] + vel[i,j]*dt + wind_disturbance[i,j]*dt + 0.5*acc[i,j]*dt*dt;

# disjunction constraints
subject to disjunction_visit_lo{i in 1..n_ts, j in 1..num_disj, k in 1..2}: pos[k,i] >= disjunction[j,k] - 1000*(1-db[i,j]);
subject to disjunction_visit_hi{i in 1..n_ts, j in 1..num_disj, k in 1..2}: pos[k,i] <= disjunction[j,k] + 1000*(1-db[i,j]);
# subject to disjunction_vel_lo{i in 1..n_ts, j in 1..num_disj, k in 1..2}: vel[k,i] >= 0 - 1000*(1-db[i,j]);
subject to disjunction_visit_logic{j in 1..num_disj}: sum{i in 1..n_ts} db[i,j] = 1;
...
# arrival check (arrival box check and arrival velocity check)
# subject to velocity_final_cst_lo{t in 1..n_ts, i in 1..2}: vel[i,t] >= vel_final[i] - 1000*(1-finish[t]);
# subject to velocity_final_cst_hi{t in 1..n_ts, i in 1..2}: vel[i,t] <= vel_final[i] + 1000*(1-finish[t]);
