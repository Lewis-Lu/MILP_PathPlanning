function ofile = integer_planner_2D_iterative_time(indicator,...
    pos_init,...
    vel_init,...
    pos_final,...
    obs_info,...
...
%
% ofile = integer_planner_2D_iterative_time(pos_init, vel_init, pos_final, wind_disturb, obs_info, opbox, num_vel_cst,...
...
% Last Modified: 22 July 2021
    ...
    disp('Mixed Integer Linear Planner for path planning. Iterative Time Method')
    ...
    % final position should be a coordinate
    assert(length(pos_final) == 2, 'Dimension of the final position box should be 2') 
%     assert(length(vel_final) == 2, 'Dimension of the final velocity should be 2')
    % assertion for constraints
    ...
    % assertion for velocity and acceleration constraints
    ...
    % indicator
    assert(floor(indicator) == indicator, 'Number of the indicator should be scalar and integer')
...
%     ctn = ctn + AMPLmatrix(fid, 'wind_disturbance', [ones(1,num_ts).*wind_x; ones(1,num_ts).*wind_y]);
    
    ctn = ctn + AMPLscalar(fid, 'epsilon', 0.001);
    ctn = ctn + AMPLscalarint(fid, 'indicator', indicator);
    ...
    ctn = ctn + AMPLvector(fid, 'vel_final', [0 0]);
    ...
    model = [model_root 'integer_planner_2D_time_iterative.mod '];
    data = [data_root 'integer_planner_2D_time_iterative.dat '];
    ofile = [output_root 'integer_planner_2D_time_iterative.txt '];
