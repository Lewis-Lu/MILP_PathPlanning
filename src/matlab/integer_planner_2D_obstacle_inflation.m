function ofile = integer_planner_2D_obstacle_inflation(pos_init, vel_init, pos_final, vel_final,...
    obs_info, opbox, num_vel_cst, num_acc_cst, num_obs_cst,...
    acc_max, vel_max, vel_min, num_ts, timeStamp, obs_inflation, out_data_file)    
% This function is integer planner in 2D environment, and the function
% returns the results from the solver.
%
% ofile = integer_planner_2D_obstacle_inflation(pos_init, vel_init, pos_final, vel_final,...
%     obs_info, opbox, num_vel_cst, num_acc_cst, num_obs_cst,...
%     acc_max, vel_max, vel_min, num_ts, timeStamp, obs_inflation, out_data_file)    
% 
...
% Last Modified: 22 Aug 2021
    ...
    assert(numel(timeStamp) >= 2,...
        'The number of the element of the time stamp array should greater or equal than 2')
    assert(length(opbox) == 4,...
        'The length of the opbox should be 4')
    assert(numel(obs_inflation) == size(obs_info, 1),...
        'The size of the obstacle inflation should equal to the number of the obstacle')
    ...
    ctn = ctn + AMPLvector(fid, 'obs_inflation', obs_inflation);
    ...
    ctn = ctn + AMPLvector(fid, 'timeStamp', timeStamp);
    ...
    model = [model_root     'integer_planner_2D_obstacle_inflation.mod '];
    data = [data_root       'integer_planner_2D_obstacle_inflation.dat '];
    ofile = [output_root    'integer_planner_2D_obstacle_inflation.txt '];
